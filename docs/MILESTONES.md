...
| Integrated Knowledge Hub | Added the Docusaurus Knowledge Hub as an eighth sidebar destination opening in a new tab at `/knowledgehub`; packaged its validated static build into the PlantMind release while preserving search and content navigation. | Hub content validation/build, app-shell unit test, type check, lint, production build and browser route checks. | COMPLETE |
