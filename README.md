...
The primary product navigation is intentionally consolidated into seven PlantMind workflow destinations plus the integrated PlantMind Knowledge Hub at `/knowledge-hub`. Executive Briefing is a view within Executive Command; investigation, approval and executive outcome are views within Decisions & Actions; Industry Solutions is entered from the executive Overview rather than the operational sidebar. The Hub is built from its Docusaurus/MDX source and packaged into the PlantMind deployment so its full documentation navigation and search remain available on the PlantMind domain.
